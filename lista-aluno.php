...
$html = file_get_html('https://sigaa.ufma.br/sigaa/public/curso/alunos_curso.jsf?lc=pt_BR&id=85784');

function num_alunos($html){
    foreach($html->find('table') as $table){
        $all_trs = $table->find('tr');
        $count = count($all_trs)-2;
        // desconta primeira e ultima linha
        return $count;
    }
}
...
for($i =1; $i<=num_alunos($html); $i++){
    echo '<tr>'."\n";
    $matricula = $html->find('table',0)->find('tr',$i)->find('td',0)->plaintext;
    $nome = $html->find('table',0)->find('tr',$i)->find('td',1)->plaintext;
